...
async function withSheets(t,fn){
  ...
  const env={
    ...
    SHEET_CONTROL_ID:'TEST_CONTROL',SHEET_ID:'TEST_A',
...
test('dual mode: exact KTP/name lookup and fuzzy matching stay in active snapshot',async t=>{
  await withSheets(t,async f=>{
    ...
    assert(f.readRequests.some(x=>x.book==='TEST_A'&&x.range.startsWith('INDEX_LEN_TOKEN!')));
    assert(!f.readRequests.some(x=>x.book==='TEST_B'));
  });
});
