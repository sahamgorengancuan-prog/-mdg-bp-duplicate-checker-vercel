...
test('health and exact response disclose running engine and index readiness without secrets', async()=>{
  ...
  assert.equal(hb.engine_version,'2026-09-23-gsheet-dual-v14-batched');
  assert.equal(hb.exact_index_ready,true);
  assert.equal(health.headers.get('x-bp-checker-engine'),'2026-09-23-gsheet-dual-v14-batched');
